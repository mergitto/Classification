...
def cross_validation(X, y):
    ...
    for max_depth in [2, 3, 4, 5, 6, 7, 8, 9]:
        ...
        viz = dtreeviz(clf, X, y, target_name='score_dummy', feature_names=list(X.keys()), class_names=["high", "low"])
...
def learning_curve_show(X, y, save_file_name):
...
def decision_tree_classifier(X, y, df, save_file_name="", learning_curve_name=""):
    # 決定木による学習
    print("============== 決定木 ===============")
    #cross_validation(X, y)
    grid_search(X, y)
    learning_curve_show(X, y, learning_curve_name)
    ...
    importance = pd.DataFrame({ '変数':X.columns, '重要度':treeModel.feature_importances_})
    print(importance)
    treeModel.fit(X, y)
    predicted = pd.DataFrame({'TreePredicted':treeModel.predict(X)})
    data_predicted = pd.concat([df, predicted], axis =1)
    if True:
        viz = dtreeviz(treeModel, X, y, target_name='score_dummy', feature_names=list(X.keys()), class_names=["high", "low"])
...
if True:
    ...
    df = decision_tree_classifier(X, y, df, save_file_name="decision_tree_questionnaire.pdf", learning_curve_name="decision_tree_image/qustion_learning_curve.pdf")
